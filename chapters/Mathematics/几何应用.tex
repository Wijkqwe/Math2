...
\subsection{平面质心坐标公式}
若密度均匀\(\rho = \)常数，则质心与形心重合。

\subsubsection{弧}
设曲线L的线密度为\(\rho(x, y)\)，弧长元\(ds\)，总质量为M，则质心坐标\((\overline{x}, \overline{y})\)公式为
\[\overline{x} = \dfrac{\displaystyle\int_Lx\rho(x, y)ds}{M}\]
\[\overline{y} = \dfrac{\displaystyle\int_Ly\rho(x, y)ds}{M}\]
\[M = \int_L\rho(x, y)ds\]


\subsubsection{平面区域}
设区域D的密度为\(\rho(x, y)\)，面积元\(dS\)，总质量为M，则质心坐标\((\overline{x}, \overline{y})\)公式为
\[\overline{x} = \dfrac{\displaystyle\iint_Dx\rho(x, y)dS}{M}\]
\[\overline{y} = \dfrac{\displaystyle\iint_Dy\rho(x, y)dS}{M}\]
\[M = \iint_D\rho(x, y)dS\]
