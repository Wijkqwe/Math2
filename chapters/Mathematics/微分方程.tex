...
\subsection{解的性质与结构}
\begin{itemize}
    \item 若解中含特解\(e^{rx}\)，则r至少为\textbf{单实根}；
    \item 若解中含特解\(x^{k - 1}e^{rx}\)，则r至少为\textbf{k重实根}；
    \item 若解中含特解\(e^{\alpha x}cos\beta x\)或\(e^{\alpha x}sin\beta x\)，则\(\alpha \pm \beta i\)至少为\textbf{单复根}；
    \item 若解中含特解\(e^{\alpha x}xcos\beta x\)或\(e^{\alpha x}xsin\beta x\)，则\(\alpha \pm \beta i\)至少为\textbf{二重复根}；

    \item 若\(y_1, ... , y_k\)是\textit{齐次方程}解，则其线性组合\(C_1y_1 + ... + C_ky_k\)也是该\textit{齐次方程}解；

    \item 若\(y_1, ..., y_n\)是\textit{n阶齐次方程}的n个线性无关的解，则其是该方程的\textbf{基本解组}，其线性组合是该方程的\textbf{通解}；
    
    \item 若\(y_1^*, y_2^*\)是\textit{非齐次方程}的\textbf{特解}，则\(y_1^* - y_2^*\)是对应\textit{齐次方程}的解；

    \item 若\(y_1^*\)是\(y'' + py' + qy = f_1(x)\)的\textbf{特解}，\(y_2^*\)是\(y'' + py' + qy = f_2(x)\)的\textbf{特解}，则\(y_1^* + y_2^*\)是\(y'' + py' + qy = f_1(x) + f_2(x)\)的\textbf{特解}；

    \item 若\(y = C_1y_1(x) + C_2y_2(x)\)是\textit{齐次方程}的\textbf{通解}，\(y^*\)是\textit{非齐次方程}的一个\textbf{特解}，则\(y + y^*\)是\textit{非齐次方程}的\textbf{通解}；
\end{itemize}
