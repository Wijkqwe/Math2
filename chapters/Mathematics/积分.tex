...
反常积分中，通常将\(\infty\)与瑕点统称为\textbf{奇点}。在判别积分敛散性时，一个积分只能有一个奇点；若出现两个及以上，需拆分。


\paragraph{比较判别法}
设函数\(f(x), g(x)\)在区间\(\begin{cases}
    [a, +\infty)\text{上连续}\ \text{(无穷区间)} \\ 
    (a, b]\text{上连续，瑕点同为}x = a\ \text{(无界函数)}
\end{cases}\)，且\(0 <= f(x) <= g(x), \begin{cases}
    a <= x < +\infty \ \text{(无穷区间)} \\ 
    a < x <= b \ \text{(无界函数)}
\end{cases}\)，则
\begin{itemize}
    \item 当\(\displaystyle\int_a^{+\infty(b)}g(x)dx\)收敛时，\(\displaystyle\int_a^{+\infty(b)}f(x)dx\)收敛；
    \item 当\(\displaystyle\int_a^{+\infty(b)}f(x)dx\)发散时，\(\displaystyle\int_a^{+\infty(b)}g(x)dx\)发散；
\end{itemize}
...
设函数\(f(x), g(x)\)在区间\(\begin{cases}
    [a, +\infty)\text{上连续}\ \text{(无穷区间)} \\ 
    (a, b]\text{上连续，瑕点同为}x = a\ \text{(无界函数)}
\end{cases}\)且\(f(x) >= 0, g(x) > 0, \displaystyle\lim_{x \to +\infty(a^+)}\dfrac{f(x)}{g(x)} = \lambda\)（有限或\(\infty\)），则
\begin{itemize}
    \item 当\(\lambda \neq 0\ \&\&\ \lambda \neq \infty\)时，\(\displaystyle\int_a^{+\infty(b)}f(x)dx\)与\(\displaystyle\int_a^{+\infty(b)}g(x)dx\)有相同的敛散性；
    \item 当\(\lambda = 0\)时，若\(\displaystyle\int_a^{+\infty(b)}g(x)dx\)收敛，则\(\displaystyle\int_a^{+\infty(b)}f(x)dx\)也收敛；
    \item 当\(\lambda = \infty\)时，若\(\displaystyle\int_a^{+\infty(b)}g(x)dx\)发散，则\(\displaystyle\int_a^{+\infty(b)}f(x)dx\)也发散；
\end{itemize}

通常有\(\begin{cases}
    \displaystyle\lim_{x \to +\infty}\dfrac{f(x)}{\frac{1}{x^p}} = \lim_{x \to +\infty}x^pf(x)\ \text{(无穷区间)} \\ 
    \displaystyle\lim_{x \to a^+}\dfrac{f(x)}{\frac{1}{(x - a)^p}} = \lim_{x \to a^+}(x - a)^pf(x)\ \text{(无界函数)}
\end{cases}\)
...
\begin{enumerate}
    ...
    \item \(\displaystyle\int_a^b\dfrac{1}{(x - a)^p}dx\text{或}\int_a^b\dfrac{1}{(b - x)^p}dx\begin{cases}
        \text{收敛，}0 < p < 1 \\ 
        \text{发散，}p >= 1
    \end{cases}\)
...
\[\int_0^{k\pi}x\sin^nxdx = \dfrac{k\pi}{2}\int_0^{k\pi}\sin^nxdx\]
\[\int_0^{k\pi}x\cos^nxdx = \dfrac{k\pi}{2}\int_0^{k\pi}\cos^nxdx\]
...
\begin{itemize}
    ...
    \item 对任何在\([0, \pi]\)上关于\(\dfrac{\pi}{2}\)对称的非负函数f(x)(\(f(\pi - x) = f(x)\))，有\[\int_0^\pi xf(x)dx = \dfrac{\pi}{2}\int_0^\pi f(x)dx\]
    \item 对区间\([a, b]\)，若存在\(f(x) = f(a + b - x)\)，则有\[\int_a^bxf(x)dx = \dfrac{a + b}{2}\int_a^bf(x)dx\]
\end{itemize}
