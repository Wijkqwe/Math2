...
\subsubsection{求等价无穷小函数}
设\(f(x) = x + a\ln(1 + x) + bx\sin x,\ g(x) = kx^3\)，若\(f(x), g(x)\)在\(x \to 0\)时是等价无穷小，求\(a, b, k\)的值。

\textbf{方法1：泰勒展开}
\[1 = \lim_{x \to 0}\dfrac{f(x)}{g(x)} = \lim_{x \to 0}\dfrac{(1 + a)x + (b - \dfrac{a}{2})x^2 + \dfrac{a}{3}x^3 + o(x^3)}{kx^3}\]
可得\(\begin{cases}
    1 + a = 0 \\
    b - \dfrac{a}{2} = 0 \\
    \dfrac{a}{3k} = 1
\end{cases}\)，故\(\begin{cases}
    a = -1 \\
    b = -\dfrac{1}{2} \\
    k = -\dfrac{1}{3}
\end{cases}\)
...
\textbf{方法2：洛必达}
\[1 = \lim_{x \to 0}\dfrac{f(x)}{g(x)} = \lim_{x \to 0}\dfrac{x + a\ln(1 + x) + bx\sin x}{kx^3} = \lim_{x \to 0}\dfrac{1 + \dfrac{a}{1 + x} + b\sin x + bx\cos x}{3kx^2}\]
由\(\displaystyle\lim_{x \to 0}3kx^2 = 0\)得\(\displaystyle\lim_{x \to 0}(1 + \dfrac{a}{1 + x} + b\sin x + bx\cos x) = \lim_{x \to 0}(1 + a) = 0\)，得\(a = -1\).
\begin{align*}
    1 & = \lim_{x \to 0}\dfrac{1 - \dfrac{1}{1 + x} + b\sin x + bx\cos x}{3kx^2} = \lim_{x \to 0}\dfrac{x + b(1 + x)\sin x + bx(1 + x)\cos x}{3kx^2(1 + x)} \\
    & = \lim_{x \to 0}\dfrac{x + b(1 + x)\sin x + bx(1 + x)\cos x}{3kx^2} \\
    & = \lim_{x \to 0}\dfrac{1 + b\sin x + b(1 + x)\cos x + b(1 + x)\cos x + bx\cos x - bx(1 + x)\sin x}{6kx}
\end{align*}
由\(\displaystyle\lim_{x \to 0}6kx = 0\)，得\[\lim_{x \to 0}[1 + b\sin x + b(1 + x)\cos x + b(1 + x)\cos x + bx\cos x - bx(1 + x)\sin x] = 0\]
即\(\displaystyle\lim_{x \to 0}(1 + 2b\cos x) = 0\)，得\(b = -\dfrac{1}{2}\)
{\footnotesize\begin{align*}
    1 & = \lim_{x \to 0}\dfrac{1 - \dfrac{1}{2}\sin x - (1 + x)\cos x - \dfrac{1}{2}x\cos x + \dfrac{1}{2}x(1 + x)\sin x}{6kx} \\
    & = \lim_{x \to 0}\dfrac{-\dfrac{1}{2}\cos x - \cos x + (1 + x)\sin x - \dfrac{1}{2}\cos x + \dfrac{1}{2}x\sin x + \dfrac{1}{2}(1 + x)\sin x + \dfrac{1}{2}x\sin x + \dfrac{1}{2}x(1 + x)\cos x}{6k} \\
    & = \dfrac{-\dfrac{1}{2}}{6k}
\end{align*}}
得\(k = -\dfrac{1}{3}\)
...
\subsubsection{单调性证明不等式}
设函数\(f(x),g(x)\)在\([a, b]\)上连续，且\(f(x)\)单调增加，\(0 <= g(x) <= 1\)，证明：
\begin{enumerate}
    \item \(0 <= \displaystyle\int_a^xg(t)dt < = x - a, x \in [a, b]\)
    \item \(\displaystyle\int_a^{a + \int_a^bg(t)dt}f(x)dx <= \int_a^bf(x)g(x)dx\)
\end{enumerate}

\textbf{解2：}
设\(p(x) = \displaystyle\int_a^xf(u)g(u)du - \int_a^{a + \int_a^xg(t)dt}f(u)du\)，则
\[p'(x) = f(x)g(x) - g(x)f[a + \int_a^xg(t)dt]\]
由(1)得\(\displaystyle\int_a^xg(t)dt < = x - a\)，故\(a + \int_a^xg(t)dt <= x\)，又\(f(x)\)单调增，故\(p'(x) >= 0\)，故\(p(x)\)单调不减，又\(p(a) = 0\)，故\(p(b) >= 0\)，得证。
...
\subsubsection{隐函数全微分}
\(z(x, y)\)由方程\(e^{x + 2y + 3z} + xyz = 1\)确定，则\(dz\bigg|_{(0, 0)} = \)

\textbf{解：}
\(x = 0, y = 0\)时\(z = 0\)，两边求偏导得
\[(3e^{x + 2y + 3z} + xy)\dfrac{\vartheta z}{\vartheta x} = -yz - e^{x + 2y + 3z}\]
\[(3e^{x + 2y + 3z} + xy)\dfrac{\vartheta z}{\vartheta y} = -xz - 2e^{x + 2y + 3z}\]
代入得\(\dfrac{\vartheta z}{\vartheta x} = -\dfrac{1}{3}, \dfrac{\vartheta z}{\vartheta y} = -\dfrac{2}{3}\)，故\(dz\bigg|_{(0, 0)} = -\dfrac{1}{3}dx - \dfrac{2}{3}dy\)
...
\subsubsection{伴随矩阵定义}
设\(A = (a_{ij})\)是三阶非零矩阵，|A|为A的行列式，\(A_{ij}\)为\(a_{ij}\)的代数余子式，若\(A_{ij} + a_{ij} = 0, (i,j = 0,1,2)\)，则\(|A| = \)\underline{-1}

\textbf{解：}
由\(A_{ij} + a_{ij} = 0\)得\(a_{ij} = -A_{ij}\)，由定义得\(A^T = -A^*\) \\
故\(|A| = |A^T| = |-A^*| = -|A|^2\)，故\(|A| = -1\).
...
\(P^{-1}AP = \begin{bmatrix}
    1 \\
     & 1 \\
     & & 2
\end{bmatrix}\)，若\(P = (a_1, a_2, a_3), Q = (a_1 + a_2, a_2, a_3)\)，则\(Q^{-1}AQ = \)
...
\(Q = P\begin{bmatrix}
    ...
    1 & 1 \\
     & & 1
\end{bmatrix}\)，故\(Q^{-1}AQ = \begin{bmatrix}
    1 \\ 
    1 & 1 \\
     & & 1
\end{bmatrix}^{-1}P^{-1}AP\begin{bmatrix}
    1 \\ 
    1 & 1 \\
     & & 1
...
设二次型f在正交变换\(x = Py\)下的标准形为\(2y_1^2 + y_2^2 - y_3^2\)，其中\(P = [e_1, e_2, e_3]\)，若\(Q = [e_1, -e_3, e_2]\)则二次型f在\(x = Qy\)下的标准形为?\underline{(\(2y_1^2 - y_2^2 + y_3^2\))}
