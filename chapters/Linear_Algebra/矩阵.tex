...
\subsection{初等变换求矩阵方程}\label{初等变换求矩阵方程}
对\(Ax = B\)\begin{itemize}
    \item 若\(A\)可逆，\(x = A^{-1}B\)
    \[\displaystyle
    \begin{bmatrix}
        A:B
    ...
    \item 若\(A_{m*n}\)不可逆（\(m < n\)）\begin{itemize}
        \item 构建增广矩阵\([A:B_{m * s}]\)
        \item 初等行变换为\([E, T_{m*(n - m)}:X]\)
        \item \(x_{n * s} = \begin{bmatrix}
            X \\
            O_{(n - m) * s}
        \end{bmatrix}\)
    \end{itemize}
...
\begin{enumerate}
    ...
    \item 两边取转置，有\(P^TA^T(P^{-1})^T = B^T, P^TA^T(P^T)^{-1} = B^T, A^T \sim B^T\)，故\(A^T,\ B^T\)相似手段与上述三种不同，如\(A^2 + A^T, B^2 + B^T\)不相似（除非A、B为\textit{可逆实对称矩阵}）
\end{enumerate}
